...
export const Card = ({ children, className = '', title, action }: CardProps) => (
  <div className={`bg-white rounded-[10px] shadow-[0_2px_6px_rgba(0,0,0,0.05)] border border-border overflow-hidden ${className}`}>
    {(title || action) && (
      <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-white">
        {title && <h2 className="text-[18px] font-semibold text-textMain">{title}</h2>}
...
export const Button = ({ children, variant = 'primary', icon: Icon, size = 'md', className = '', ...props }: ButtonProps) => {
  ...
  const variants = {
    ...
    secondary: "bg-surfaceHighlight text-primary border border-secondary hover:bg-[#d8efe2] focus:ring-secondary",
    danger: "bg-statusRejected hover:bg-[#a63a3a] text-white focus:ring-statusRejected",
    ghost: "bg-transparent text-textSec hover:text-textMain hover:bg-gray-100"
  };

  return (
    <button className={`${baseStyles} ${sizeStyles} ${variants[variant]} ${className}`} {...props}>
...
export const Input = ({ label, error, className = '', ...props }: InputProps) => (
  <div className="flex flex-col gap-1.5 w-full">
    {label && <label className="text-[14px] text-textSec font-medium">{label}</label>}
    <input
...
// --- Table Components ---
export const Table = ({ children, ...props }: React.ComponentProps<'table'>) => (
...
export const Thead = ({ children, ...props }: React.ComponentProps<'thead'>) => (
  <thead className="bg-surfaceHighlight text-textMain text-[13px] font-medium border-b border-border" {...props}>
    {children}
  </thead>
);

export const Th = ({ children, className = '', ...props }: React.ComponentProps<'th'>) => (
  <th className={`px-4 py-3 whitespace-nowrap ${className}`} {...props}>{children}</th>
);
