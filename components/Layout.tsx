...
import { LayoutDashboard, FileText, Settings, LogOut, User as UserIcon, Building2, ScrollText, HardHat, ClipboardCheck } from 'lucide-react';
      ...
      <aside className="w-64 bg-white border-r border-border flex flex-col fixed h-full z-10">
        <div className="h-16 flex items-center px-6 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-textMain tracking-tight">MEDCHECK</span>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <Link to={isDirector ? "/director" : "/site"} 
        ...
        <div className="p-4 border-t border-border">
          <div className="flex items-center gap-3 px-2 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-textMain truncate">{currentUser.name}</p>
              <p className="text-xs text-textSec truncate">{isDirector ? 'Diretoria' : 'Eng. Obra'}</p>
            </div>
            <button onClick={handleLogout} className="text-textSec hover:text-statusRejected p-1" title="Sair">
