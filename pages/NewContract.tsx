...
export const NewContract = () => {
  ...
  const { currentUser, sites, suppliers, addContract, addSupplier } = useAppContext();
  ...
  const [items, setItems] = useState<NewItemDraft[]>([
    { id: '1', description: '', unit: '', quantityContracted: '', unitPriceMaterial: '', unitPriceLabor: '' }
  ]);
  ...
  const totalContractValue = items.reduce((acc, item) => acc + calculateTotal(item), 0);
  ...
  const handleSaveContract = () => {
    ...
    if (items.some(i => !i.description || !i.quantityContracted || (!i.unitPriceMaterial && !i.unitPriceLabor))) {
    ...
    <div className="space-y-6 pb-20 relative">
      <header className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-textSec">Cadastre um novo contrato e seus itens para medição.</p>
        </div>
      </header>
      ...
      <Card title="Dados do Contrato">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
               <label className="text-sm font-medium text-textSec">Fornecedor *</label>
               <div className="flex gap-2">
                 <select 
                  ...
                  >
                    ...
                    {suppliers.map(s => (
                      <option key={s.id} value={s.id}>{s.corporateName}</option>
                    ))}
                  </select>
                  <Button type="button" variant="secondary" onClick={() => setShowSupplierModal(true)} className="w-[38px] px-0 flex items-center justify-center">
            ...
            {selectedSupplier && (
               <div className="bg-gray-50 p-3 rounded-md border border-gray-200">
                  <span className="text-xs text-textSec block">Documento Selecionado</span>
                  <span className="font-mono text-sm font-medium text-textMain">{selectedSupplier.cnpj}</span>
               </div>
            )}
          </div>
      ...
      <Card title="Itens da Planilha Contratual">
               ...
               <Tr key={item.id}>
                 ...
                 <Td className="p-2">
                    <input 
                       ...
                       onChange={(e) => updateItem(item.id, 'quantityContracted', e.target.value)}
                    />
                 </Td>
                 <Td className="p-2">
                    <input 
                       ...
                       onChange={(e) => updateItem(item.id, 'unitPriceMaterial', e.target.value)}
                    />
                 </Td>
                 <Td className="p-2">
                    <input 
                       ...
                       onChange={(e) => updateItem(item.id, 'unitPriceLabor', e.target.value)}
                    />
                 </Td>
                 ...
                 <Td className="text-center">
                    <button onClick={() => removeItemRow(item.id)} className="text-gray-400 hover:text-red-500">
         ...
         <div className="p-4 bg-gray-50 border-t border-border flex justify-between items-center">
            ...
            <div className="text-right">
               <span className="text-sm text-textSec mr-2">Valor Total do Contrato:</span>
               <span className="text-xl font-bold text-primary">{formatCurrency(totalContractValue)}</span>
            </div>
         </div>
      </Card>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 animate-in fade-in zoom-in duration-200">
              <div className="flex justify-between items-center mb-4 border-b border-border pb-2">
                 <h3 className="text-lg font-bold text-textMain">Novo Fornecedor</h3>
                 <button onClick={() => setShowSupplierModal(false)} className="text-textSec hover:text-red-500">
                    <X className="w-5 h-5" />
                 </button>
              </div>
              <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-6">
                 ...
                 <Button variant="primary" onClick={handleSaveSupplier}>Cadastrar</Button>
              </div>
           </div>
        </div>
      )}
