...
export const SiteDashboard = () => {
  const { currentUser, sites, contracts, suppliers, getEnrichedMeasurements } = useAppContext();
  const navigate = useNavigate();

  // Filter sites this user is linked to
  const linkedSites = sites.filter(s => currentUser?.linkedConstructionSiteIds?.includes(s.id));
  const [selectedSiteId, setSelectedSiteId] = useState<string>(linkedSites[0]?.id || '');

  // Derived data
  const filteredContracts = contracts.filter(c => c.constructionSiteId === selectedSiteId && c.status === 'ATIVO');
  const allMeasurements = getEnrichedMeasurements();
  const siteMeasurements = allMeasurements.filter(m => m.contract.constructionSiteId === selectedSiteId);

  // Contracts logic with progress
  const contractsWithData = filteredContracts.map(c => {
    const sup = suppliers.find(s => s.id === c.supplierId);
    const ms = allMeasurements.filter(m => m.contractId === c.id && m.status === 'APROVADA');
    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-textSec">Gerencie medições e contratos da sua obra.</p>
        </div>
          ...
          <tbody>
            {contractsWithData.map(c => (
              <Tr key={c.id}>
                ...
                <Td className="text-right text-textSec">{formatCurrency(c.measured)}</Td>
                <Td className="text-center">
                  <span className={`text-xs font-bold px-2 py-1 rounded ${c.pct >= 100 ? 'bg-green-100 text-green-700' : 'bg-gray-100'}`}>
                    {c.pct.toFixed(1)}%
                  </span>
                </Td>
                <Td className="text-center">
                  <Button variant="ghost" size="sm" onClick={() => navigate('/new-measurement')}>
                    Medir
                  </Button>
                </Td>
              </Tr>
        ...
        <Table>
          ...
          <tbody>
            {siteMeasurements.length === 0 && (
              <Tr><Td className="text-center py-4 text-textSec" colSpan={6}>Nenhuma medição encontrada.</Td></Tr>
            )}
              ...
              <Tr key={m.id} onClick={() => navigate(`/measurement/${m.id}`)}>
                ...
                <Td>
                  <div className="flex flex-col">
                    <span className="font-medium">{m.supplier.corporateName}</span>
                    <span className="text-xs text-textSec">{m.contract.object}</span>
                  </div>
                </Td>
                <Td className="text-right font-medium">{formatCurrency(m.totalValue)}</Td>
                <Td className="text-center"><Badge status={m.status} /></Td>
                <Td className="text-center">
                  <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); navigate(`/measurement/${m.id}`); }}>
                    Detalhes
                  </Button>
                </Td>
              </Tr>
