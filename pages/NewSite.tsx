...
import { Card, Button, Input, Table, Thead, Th, Tr, Td, Badge } from '../components/UI';
    ...
    <div className="space-y-6 pb-20 relative">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-textMain">Gestão de Obras</h1>
            <p className="text-textSec">Visualize as obras cadastradas ou adicione novos empreendimentos.</p>
          </div>
        </div>
      ...
      <Card>
        <Table>
           ...
           <tbody>
              {sites.map(site => (
                 <Tr key={site.id}>
                    <Td className="font-mono text-xs text-textSec">{site.code || '-'}</Td>
                    ...
                    <Td className="text-center"><Badge status={site.status} /></Td>
                 </Tr>
              ))}
              {sites.length === 0 && (
                 <Tr><Td colSpan={5} className="text-center py-8 text-textSec">Nenhuma obra cadastrada.</Td></Tr>
              )}
           </tbody>
        </Table>
      </Card>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 animate-in fade-in zoom-in duration-200">
              <div className="flex justify-between items-center mb-6 border-b border-border pb-4">
                 ...
                 <button onClick={() => setShowModal(false)} className="text-textSec hover:text-red-500">
                    <X className="w-5 h-5" />
                 </button>
              </div>
              ...
              <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-border">
                 ...
                 <Button variant="primary" onClick={handleSave}>Salvar Obra</Button>
              </div>
           </div>
        </div>
      )}
