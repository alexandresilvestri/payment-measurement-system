...
export const NewMeasurement = () => {
  ...
  const { currentUser, contracts, suppliers, sites, measurements, addMeasurement } = useAppContext();
  ...
  const availableContracts = useMemo(() => {
    ...
    return contracts.filter(c => c.constructionSiteId === selectedSiteId && c.status === 'ATIVO');
  }, [selectedSiteId, contracts]);

  const selectedContract = contracts.find(c => c.id === selectedContractId);
  // Re-fetch site and supplier to ensure we display correct info regardless of context
  const selectedSupplier = selectedContract ? suppliers.find(s => s.id === selectedContract.supplierId) : null;
  const selectedSite = selectedContract ? sites.find(s => s.id === selectedContract.constructionSiteId) : null;
  ...
  const totalMeasurementValue = contractMath.reduce((acc, item) => acc + item.currentTotal, 0);
  ...
  const previousAccumulatedValue = useMemo(() => {
    ...
    return measurements
      .filter(m => m.contractId === selectedContract.id && m.status === 'APROVADA')
  ...
  const handleSave = (status: Measurement['status']) => {
    ...
    const newMeasurement: Measurement = {
      ...
      number: measurements.filter(m => m.contractId === selectedContract.id).length + 1,
    ...
    if(status === 'PENDENTE' && isDirector) {
       alert("Medição criada com sucesso! Ela estará disponível na sua lista de aprovações.");
    }
    ...
    <div className="space-y-6 pb-20">
      <header className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-textSec">Selecione a obra, o contrato e informe as quantidades.</p>
        </div>
      </header>
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col gap-4">
             ...
             {/* 2. Contract Selector */}
             <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-textSec">Contrato</label>
                <select 
                ...
                >
                  <option value="">
                     {!selectedSiteId ? 'Selecione uma obra primeiro...' : 'Selecione o contrato...'}
                  </option>
          ...
          {selectedContract && (
             <div className="bg-gray-50 p-4 rounded-lg border border-border flex flex-col justify-center">
                <h3 className="font-semibold text-textMain mb-3 border-b border-gray-200 pb-2">Resumo Financeiro do Contrato</h3>
                <div className="grid grid-cols-3 gap-4">
                    <div>
                      <span className="block text-xs text-textSec">Valor Contrato</span>
                      <span className="font-semibold text-textMain">{formatCurrency(totalContractedValue)}</span>
                    </div>
                    <div>
                      <span className="block text-xs text-textSec">Acumulado Anterior</span>
                      <span className="font-semibold text-textMain">{formatCurrency(previousAccumulatedValue)}</span>
                    </div>
                    <div>
                      <span className="block text-xs text-textSec">Saldo Contratual</span>
                      <span className="font-semibold text-primary">{formatCurrency(totalContractedValue - previousAccumulatedValue)}</span>
                    </div>
                </div>
             </div>
          )}
          ...
          <Card title="Itens do Contrato">
              ...
              <tbody>
                {contractMath.map(item => (
                  <Tr key={item.id} className={!item.isValid ? 'bg-red-50' : ''}>
                    <Td className="font-medium text-textMain">{item.description}</Td>
                    <Td className="text-center text-textSec">{item.unit}</Td>
                    <Td className="text-right">{item.quantityContracted}</Td>
                    <Td className="text-right text-textSec">{item.accumulatedQty}</Td>
                    <Td className="text-right font-medium text-primary">{item.balanceQty}</Td>
                    <Td className="bg-green-50 p-2">
                       <input 
                         type="number" 
                         min="0"
                         max={item.balanceQty}
                         className={`w-full text-right p-1 border rounded focus:ring-2 focus:ring-primary outline-none ${!item.isValid ? 'border-red-500 text-red-600' : 'border-gray-300'}`}
                         value={item.currentQty || ''}
                         onChange={(e) => handleQuantityChange(item.id, e.target.value)}
                         placeholder="0"
                       />
                       {!item.isValid && <div className="text-[10px] text-red-500 text-right">Excede saldo</div>}
                    </Td>
                    <Td className="text-right text-textSec">{formatCurrency(item.unitPriceTotal)}</Td>
                    <Td className="text-right font-bold text-textMain">{formatCurrency(item.currentTotal)}</Td>
                  </Tr>
                ))}
              </tbody>
            </Table>
            ...
            <div className="p-4 bg-surfaceHighlight border-t border-border mt-4 flex justify-end items-center gap-6">
              <div className="text-right">
                <span className="block text-sm text-textSec">Total desta Medição</span>
                <span className="text-2xl font-bold text-primary">{formatCurrency(totalMeasurementValue)}</span>
              </div>
            </div>
          </Card>
