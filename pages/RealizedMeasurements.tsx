...
export const RealizedMeasurements = () => {
  ...
  const { currentUser, sites, suppliers, getEnrichedMeasurements } = useAppContext();
  ...
  const availableSites = isDirector 
    ? sites 
    : sites.filter(s => currentUser?.linkedConstructionSiteIds?.includes(s.id));
  ...
  const realizedMeasurements = useMemo(() => {
    ...
    if (selectedSiteId) {
      list = list.filter(m => m.contract.constructionSiteId === selectedSiteId);
    } else {
  ...
  const availableSuppliersForSite = useMemo(() => {
    ...
    const relevantSupplierIds = new Set(
        ...
        .filter(m => m.status === 'APROVADA' && m.contract.constructionSiteId === selectedSiteId)
  ...
  return (
    <div className="space-y-6 pb-20">
      <header>
        <h1 className="text-2xl font-bold text-textMain">Medições Realizadas</h1>
        <p className="text-textSec">Histórico de medições aprovadas e encerradas.</p>
      </header>
      ...
      <Card>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
            <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-textSec">Selecione a Obra</label>
                <select 
            ...
            <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-textSec">Filtrar por Fornecedor (Opcional)</label>
                <select 
      ...
      {/* Results Table */}
      {selectedSiteId && (
        <Card title={`Medições Aprovadas - ${availableSites.find(s => s.id === selectedSiteId)?.name}`}>
            <Table>
                <Thead>
                    <Tr>
                        <Th>Nº</Th>
                    ...
                    {realizedMeasurements.length === 0 ? (
                        <Tr>
                            <Td colSpan={6} className="text-center py-8 text-textSec">
                                Nenhuma medição aprovada encontrada para os filtros selecionados.
                            </Td>
                        </Tr>
                    ) : (
                        realizedMeasurements.map(m => (
                            <Tr key={m.id} onClick={() => navigate(`/measurement/${m.id}`)}>
                                ...
                                <Td className="text-sm text-textSec">{m.contract.object}</Td>
                                <Td className="text-right font-medium text-primary">{formatCurrency(m.totalValue)}</Td>
      ...
      {!selectedSiteId && (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-lg border border-dashed border-gray-300">
              ...
              <p className="text-textSec font-medium">Selecione uma obra acima para visualizar o histórico de medições.</p>
          </div>
      )}
    </div>
  );
